...
final readonly class DatabaseParentEvidence
{
    private function __construct(
        ...
        if ($address !== null && $address->type !== ResourceType::DATABASE_CLUSTER) {
            throw new InvalidArgumentException('Logical Database parent evidence requires a Database Cluster address.');
        }
        if (($address === null) !== ($remoteId === null)) {
            throw new InvalidArgumentException('Resolved Database parent evidence requires both local and remote identity.');
        }
        if ($remoteId !== null && trim($remoteId) === '') {
            throw new InvalidArgumentException('Resolved Database parent identity must not be empty.');
        }
        if ($remoteId === null && $completeness !== EvidenceStatus::INCOMPLETE) {
            throw new InvalidArgumentException('An unresolved Database parent requires incomplete evidence.');
        }
    }
